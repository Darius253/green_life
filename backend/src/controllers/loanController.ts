...
import { caclPerform, calcLoanperfomance, retLimit, retQuery } from '../utils/Sanitize';
...
export const repaymentHook = async (req: Request, res: Response) => {
  ...
  if (amount <= currentInstallment.remainingBalance) {
    ...
    if (amount === currentInstallment.remainingBalance && loan.installment.length < loan.loanterm) {
    ...
    currentInstallment.performance = caclPerform(
      amount,
      currentInstallment.remainingBalance , currentInstallment.performance!
    );
    currentInstallment.remainingBalance = new Decimal(currentInstallment.remainingBalance - amount).toDecimalPlaces(2).toNumber();
    if (currentInstallment.remainingBalance <= 0) {
    ...
    currentInstallment.remainingBalance = 0.00
    currentInstallment.lastPaymentDate = moment().toDate();
    ...
    while (x > 0 && loan.installment.length < loan.loanterm) {
      ...
      loan.installment.push({
        ...
        remainingBalance:
          x > loan.monthlyPayment ? 0.0 : new Decimal(loan.monthlyPayment - x).toDecimalPlaces(2).toNumber(),
        status:
          x > loan.monthlyPayment
            ? loanInstallmentStatus.SETTLED
            : loanInstallmentStatus.UNPAID,
        performance: x > loan.monthlyPayment ? 100 : caclPerform(x, loan.monthlyPayment ,0),
        latePayment: false,
        lastPaymentDate: moment().toDate(),
      });
  ...
  loan.remainingBalance = new Decimal(loan.remainingBalance - amount).toDecimalPlaces(2).toNumber()

  loan.lastRepaymentDate = moment().toDate();
  loan.performance = calcLoanperfomance([...loan.installment]);
